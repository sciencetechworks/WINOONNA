...
// Devuelve un tiempo de profile en segundos
double backpropagation_ejemplo_predice_seno(int n_hidden_neurons,int max_epochs,double alpha)
{
	...
	PatternCollection *pcollection=new PatternCollection();
	...
	PatternCollection *targetcollection=new PatternCollection();
	...
	cout<<"BackPropagation EXAMPLE SIN  RUNNING: TRAINING NET"<<endl;


	BackPropagation backpropagation(10,n_hidden_neurons,1);

	//backpropagation.setLearningFactor(1.0/(n_hidden));
	backpropagation.setLearningFactor(alpha);
	...
	backpropagation.training_algorithm->setMax_epochs(max_epochs);
	backpropagation.logger.disable();
	...
	double segundos=-1.0;
	timer.start();
	((BackPropagationTrainingAlgorithm*)backpropagation.training_algorithm)->train();
	timer.stop();
	segundos=timer.get_seconds();

	// DESACTIVADA PARA EL EJEMPLO, ACTIVAR!
...
double backpropagation_ejemplo_predice_seno_omp(int n_hidden_neurons,int max_epochs,double alpha,int nthreads)
{
	...
	double segundos=-1.0;
	timer.start();
	((BackPropagationTrainingAlgorithm*)backpropagation.training_algorithm)->train_omp(nthreads);
	timer.stop();
	segundos=timer.get_seconds();
...
void backpropagation_ejemplo_seno_genera_graficas(char* result_prediction_file_path)
{
	...
	ofstream fout;
	fout.open(result_prediction_file_path);
	double x=0.1;
