...
{
	vector<Neuron*>::iterator niter=neurons_list.begin();
	...
	vector<Neuron*>::iterator biter=biases_list.begin();
	...
	vector<Sinapsis*>::iterator current = this->sinapsis_list.begin();
...
{
	vector<Neuron*>::iterator iter=biases_list.begin();
...
{
	vector<Neuron*>::iterator iter = this->neurons_list.begin();
	int i=0;
...
{
	/*vector<Neuron*>::iterator iter = this->biases_list.begin();
	while (iter!=biases_list.end())
	{
		if (*iter==neuron)
			return true;
...
bool NeuralNet::save(const char *path)
{
	...
	fout<<"<NEURAL_NET>"<<endl;
		...
		fout<<"</NEURONS>"<<endl;
		fout<<"<BIAS_LIST>"<<endl;
			vector<Neuron*> biaslist=biases_list;
			vector<Neuron*>::iterator biter = biaslist.begin();
		...
		fout<<"<INPUT_LAYER>"<<endl;
			vector<Neuron*> neuronlist=this->input_layer->getNeuron_list();
			vector<Neuron*>::iterator iter = neuronlist.begin();
		...
		fout<<"<SINAPSIS_LIST>"<<endl;
			vector<Sinapsis*> sinapsislist=this->sinapsis_list;
			vector<Sinapsis*>::iterator siter = sinapsislist.begin();
...
void NeuralNet::predict()
{
	...
	while(layer_iter!=hidden_layers.end())
	{
	 ...
	 vector<Neuron*>::iterator neuron_iter = neuronlist.begin();
	...
	//CAPA DE SALIDA
	vector<Neuron*> neuronlist=this->output_layer->getNeuron_list();
	vector<Neuron*>::iterator iter = neuronlist.begin();
	while(iter!=neuronlist.end())
	{
	 Neuron *neuron=*iter;
...
{
	vector<Neuron*> neuronlist=this->neurons_list;
	vector<Neuron*>::iterator iter = neuronlist.begin();
	while(iter!=neuronlist.end())
	{
		Neuron *neuron=*iter;
...
void NeuralNet::setInput(Pattern *pattern)
{
	/*Recorremos la capa de entrada asignando valores de entrenamiento*/
		vector<Neuron*> input_neuron_list=input_layer->getNeuron_list();
...
Pattern *NeuralNet::getOutput(int rows,int cols)
{
	...
	list<double> results;
		...
		vector<Neuron*> output_neurons_list=output_layer->getNeuron_list();
