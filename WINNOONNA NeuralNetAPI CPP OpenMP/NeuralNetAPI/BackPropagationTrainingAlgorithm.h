...
class BackPropagationTrainingAlgorithm :
	public TrainingAlgorithm
{

public:
	bool init_weights_to_random_values;
	double objective_min_cuadratic_error;
	
	BackPropagationTrainingAlgorithm(void);
	virtual ~BackPropagationTrainingAlgorithm(void);
	BackPropagationTrainingAlgorithm(NeuralNet *the_net);

	virtual bool stop_condition();
	virtual void train();
	virtual void train_omp(int nthreads);
...
private:
	int current_epoch;
	void fill_weights_with_little_random_values();
	void NW_weights_initialization();

	void fill_weights_with_little_random_values_omp();
	void NW_weights_initialization_omp();

	double median_cuadratic_error_in_this_epoch;
