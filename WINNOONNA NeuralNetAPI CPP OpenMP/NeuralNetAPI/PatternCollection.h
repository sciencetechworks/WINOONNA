...
{
public:
	list<Pattern*> patterns_list;
	PatternCollection(void);
	virtual ~PatternCollection(void);

	bool load(const char*path);

	double getMaxValueInPatterns();
	void print();
	void add_pattern(Pattern *pattern);
	void empty_training_patterns();

	void convert_all_patterns_to_bipolar();

	void PatternCollection::generate_from_data(char *dataPath,int patlength,int step);

	double applied_normalization_factor;
