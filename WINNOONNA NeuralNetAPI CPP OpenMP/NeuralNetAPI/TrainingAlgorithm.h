...
public:
	 NeuralNet *net;
	 PatternCollection training_patterns;
	 PatternCollection target_patterns;

	TrainingAlgorithm(void);
	virtual ~TrainingAlgorithm(void);

	void setMax_epochs(int n);

	TrainingAlgorithm(NeuralNet *the_net);
	
	double calculate_hit_ratio_after_training(int noise_percent,double max_allowed_dist_to_target);
