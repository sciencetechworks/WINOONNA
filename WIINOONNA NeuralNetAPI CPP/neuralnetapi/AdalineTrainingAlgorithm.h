...
class AdalineTrainingAlgorithm :
	public TrainingAlgorithm
{
private:
    double commited_error;
	double objective_min_error;
	double max_weight_change;
	double objective_min_weight_variation;

public:
	bool init_weights_to_random_values;
	
	AdalineTrainingAlgorithm(void);
	~AdalineTrainingAlgorithm(void);
	AdalineTrainingAlgorithm(NeuralNet *the_net);

	virtual bool stop_condition();
	virtual void train();

	void setObjective_min_error(double value);
	void setObjective_min_weight_variation(double value);

private:
	
	void fill_weights_with_little_random_values();
	double  caudratic_error_to_target(double y_in,double target);
