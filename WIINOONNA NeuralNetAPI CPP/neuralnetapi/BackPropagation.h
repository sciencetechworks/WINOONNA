...
class BackPropagation:
	public NeuralNet
{
friend class BackPropagationTransferFunction;
friend class BackPropagationLearningFunction;
friend class BackPropagationTrainingAlgorithm;
public:
	bool map_results_to_0_1; //mapear a valores puros 0 y 1 
	BackPropagation(void);
	~BackPropagation(void);
	BackPropagation(int n_inputs,int m_hidden,int n_outputs);

	void setLearningFactor(double alpha);
