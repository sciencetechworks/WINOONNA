...
PerceptronTransferFunction::PerceptronTransferFunction(void)
{
}
...
void PerceptronTransferFunction::setThreshold(double theta)
{
	threshold=theta;
}

void PerceptronTransferFunction::calculate_output(Neuron *neuron)
{
	...
	if (y_input>threshold)
	{
		neuron->setFired_output(1.0);
	}else
	if ((y_input>=-threshold)&&(y_input<=threshold))
	{
